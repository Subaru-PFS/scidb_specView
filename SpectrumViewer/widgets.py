...
class CheckButtons(AxesWidget):
    ...
    """
    def __init__(self, ax, labels, x1=0.0, y1=1.0, actives=None, colors=None):
        """
        ...
        if len(labels)*dy > 1:
            ...
            ys = np.linspace(y1 - dy, dy, len(labels))
        else:
            #ys = [ 1.0 - dy - i*dy   for i in range(len(labels))   ]
            ys = [y1 - dy - i * dy for i in range(len(labels))]
        ...
        x_text = x1 + 0.04
        x_checkbutton = x1 + 0.00

        for y, label, active, color in zip(ys, labels, actives, colors):
            t = ax.text(x_text, y, label, transform=ax.transAxes,
                        horizontalalignment='left',
                        verticalalignment='center', color=color)

            w, h = dy / 2, dy / 2
            x, y = x_checkbutton, y - h / 2

            #p = Rectangle(xy=(x, y), width=w, height=h, edgecolor='black',
            #              facecolor=axcolor, transform=ax.transAxes)
            p = Rectangle(xy=(x, y), width=w, height=h, edgecolor=color,
                          facecolor=axcolor, transform=ax.transAxes)
